...
function renderOverview(day, row, el) {
  ...
  const rows = trailingRows(state.current, 14);
  lineChart("ov-kcal",
...
function renderRetainerStrip() {
  const rows = trailingRows(state.current, 30).filter((r) => r.retainers_worn !== null);
  ...
  return `<div style="display:flex;gap:4px;flex-wrap:wrap">${rows.map((r) =>
    `<div title="${r.date}: ${r.retainers_worn ? "worn" : "skipped"}" style="width:18px;height:18px;border-radius:4px;background:${r.retainers_worn ? COLORS.green : COLORS.red}"></div>`
  ).join("")}</div>`;
}

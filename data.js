...
const EXERCISE_LIBRARY = {
  ...
  "Hack Squat":             { kind: "strength", muscles: { quads: 1.0, glutes: 0.4 } },
...
const SNAPSHOT = [
  ...
  { name: "Dumbbell Pullover",       section: "Back", latest: { sets: 4, reps: 9, weight: 22.5, text: "22.5kg × 4×9 (new PR)" }, best: { sets: 4, reps: 9, weight: 22.5, text: "22.5kg × 4×9" } },
  { name: "Dead Hang",               section: "Back", iso: true, latest: { scheme: [ { sets: 1, seconds: 30, weight: 81.00 } ], text: "30s (1 set, abbreviated)" }, best: { scheme: [ { sets: 1, seconds: 35, weight: 81.00 }, { sets: 1, seconds: 30, weight: 81.00 }, { sets: 1, seconds: 25, weight: 81.00 } ], text: "35s + 30s + 25s" } },
...
const WORKOUTS = [
  ...
  {
    datetime: "2026-09-16T10:45", note: "Hotel-gym session — Lower Body + Back (dumbbells to 20/22.5kg, kettlebells 8/12/16kg, one adjustable dual-pulley cable station, plus a seated leg extension machine and a lat pulldown machine). Departed from the pre-set plan (which was hamstring/glute-biased with cable-station substitutes) in favor of whatever equipment was actually available. Leg Extensions 50kg×4×10 (hotel machine's marker reading — user flagged the number as implausible/felt much harder than 50kg, likely a differently-calibrated stack; not comparable to home's 111kg machine, logged for session-load tracking only, not a PR read); Lat Pulldown (Wide Grip) 75kg×3×8 (correction 2026-09-18 — originally mislogged under Lat Pulldown (Triangle), but this hotel machine was actually a wide-grip attachment, not the close-neutral Triangle grip; first logged instance of this exercise, so both latest and best); Dumbbell Pullover 22.5kg×4×8 (new PR — est. 1RM 28.5 vs 28.0 for the 20kg×4×12 best); Dead Hang 35s+30s+25s (ties best exactly); Dumbbell RDL 22.5kg each (45kg total)×4×6 (new PR by load, but a different execution — two dumbbells, one per hand, vs the previous single-DB-both-hands style, so not a strictly equivalent progression); Incline Pec Fly 15kg each (30kg)×4×8 (below the 36kg/18kg-each best, not a PR — hotel dumbbell selection). Also did Rope Cable Extension on an offset-weighted cable machine, described only as 'Heavy' with no numeric marker available — not logged as a quantified set to avoid corrupting the real calibrated home PR (75kg×4×8) with an unverifiable number. 1 new PR + 1 first-time log (execution-caveated).",
    exercises: [
      { name: "Leg Extensions",           sets: [ { reps: 10, weight: 50 }, { reps: 10, weight: 50 }, { reps: 10, weight: 50 }, { reps: 10, weight: 50 } ] },
      { name: "Lat Pulldown (Wide Grip)", sets: [ { reps: 8, weight: 75 }, { reps: 8, weight: 75 }, { reps: 8, weight: 75 } ] },
  ...
  // 11 Sep — Outdoor Run, Or Akiva (15:00, Nike Run Club)
  {
    ...
    exercises: [
      { name: "Outdoor Run", distanceKm: 3.51, durationMin: 28.4, cadence: 138, elevationGainM: 23 },
    ],
  },
  ...
  // 2 Sep — Outdoor Walk, Or Akiva (17:15, concluded 17:45)
  {
    ...
    exercises: [
      { name: "Outdoor Walk", distanceKm: 2.2, durationMin: 30 },
    ],
  },
  // 1 Sep — Outdoor Walk, suburban (20:50, concluded 21:20)
  {
    datetime: "2026-09-01T20:50", note: "Outdoor Walk — suburban, concluded 21:20",
    exercises: [
      { name: "Outdoor Walk", distanceKm: 2.2, durationMin: 30 },
    ],
  },
  ...
  // 31 Aug — Back day, as suggested (20:30)
  {
    ...
    exercises: [
      { name: "Diverging Seated Row",   sets: [ { reps: 10, weight: 97 }, { reps: 10, weight: 97 }, { reps: 10, weight: 97 }, { reps: 10, weight: 97 } ] },
      { name: "Low Row",                sets: [ { reps: 11, weight: 79 }, { reps: 11, weight: 79 }, { reps: 11, weight: 79 }, { reps: 11, weight: 79 } ] },
      { name: "Reverse Incline DB Row", sets: [ { reps: 8, weight: 56 }, { reps: 8, weight: 56 }, { reps: 8, weight: 56 }, { reps: 8, weight: 56 } ] },
      { name: "Dumbbell Pullover",      sets: [ { reps: 8, weight: 22 }, { reps: 8, weight: 22 }, { reps: 8, weight: 22 }, { reps: 8, weight: 22 } ] },
      { name: "Dead Hang",              sets: [ { seconds: 35 }, { seconds: 30 }, { seconds: 25 } ] },
    ],
  },
  // 29 Aug — Outdoor Walk (08:40, concluded 10:00)
  {
    datetime: "2026-08-29T08:40", note: "Outdoor Walk — arms/legs sun exposure with 34 SPF sunscreen",
    exercises: [
      { name: "Outdoor Walk", distanceKm: 6, durationMin: 80 },
    ],
  },
  ...
  // 24 Aug — Legs & Glutes, one-off scheme with wife (21:05)
  {
    ...
    exercises: [
      { name: "Leg Extensions",   sets: [ { reps: 8, weight: 111 }, { reps: 8, weight: 111 }, { reps: 8, weight: 111 } ] },
      { name: "Leg Curls",        sets: [ { reps: 8, weight: 111 }, { reps: 8, weight: 111 }, { reps: 8, weight: 111 } ] },
      { name: "Inner Thigh",      sets: [ { reps: 9, weight: 79 }, { reps: 9, weight: 79 }, { reps: 9, weight: 79 } ] },
      { name: "Outer Thigh",      sets: [ { reps: 9, weight: 79 }, { reps: 9, weight: 79 }, { reps: 9, weight: 79 } ] },
      { name: "Glute Extension",  sets: [ { reps: 6, weight: 32 }, { reps: 6, weight: 32 }, { reps: 6, weight: 32 } ] },
      { name: "Leg Press",        sets: [ { reps: 10, weight: 132 }, { reps: 10, weight: 132 }, { reps: 10, weight: 132 } ] },
      { name: "Calf Raises Machine", sets: [ { reps: 6, weight: 83.5 }, { reps: 6, weight: 83.5 }, { reps: 6, weight: 83.5 } ] },
      { name: "Angled Leg Press", sets: [ { reps: 8, weight: 152 }, { reps: 8, weight: 152 }, { reps: 8, weight: 152 } ] },
      { name: "Hip Thrust",       sets: [ { reps: 8, weight: 81.1 }, { reps: 8, weight: 81.1 }, { reps: 8, weight: 81.1 } ] },
      { name: "Hack Squat",       sets: [ { reps: 6, weight: 123.7 }, { reps: 6, weight: 123.7 }, { reps: 6, weight: 123.7 } ] },
    ],
  },
